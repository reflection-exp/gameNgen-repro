    ...
    [
        transforms.Resize(
            ...
            interpolation=getattr(
                transforms.InterpolationMode, IMG_INTERPOLATION.upper()
            ),
        ),
        transforms.ToTensor(),
...
def collate_fn(examples):
    ...
    # TODO: UGLY HACK
    images = no_img_conditioning_augmentation(
        images, prob=ZERO_OUT_ACTION_CONDITIONING_PROB
    )
    return {
        "pixel_values": images,
        "input_ids": torch.stack(
            [
                example["input_ids"][: BUFFER_SIZE + 1].clone().detach()
                for example in examples
            ]
        ),
    }
...
def preprocess_train(examples):
    ...
    actions = (
        torch.tensor(examples["actions"])
        if isinstance(examples["actions"], list)
        else examples["actions"]
    )
...
class EpisodeDataset:
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        if idx < BUFFER_SIZE:
            padding = [
                IMG_TRANSFORMS(Image.new("RGB", (WIDTH, HEIGHT), color="black"))
                for _ in range(BUFFER_SIZE - idx)
            ]
            return {
                "pixel_values": padding + self.dataset[: idx + 1]["pixel_values"],
                "input_ids": torch.concat(
                    [
                        torch.zeros(len(padding), dtype=torch.long),
                        self.dataset[: idx + 1]["input_ids"],
                    ]
                ),
            }
...
def get_dataloader(
    dataset_name: str, batch_size: int = 1, num_workers: int = 1, shuffle: bool = False
...
def get_single_batch(dataset_name: str) -> dict[str, torch.Tensor]:
    dataloader = get_dataloader(
        dataset_name, batch_size=1, num_workers=1, shuffle=False
    )
