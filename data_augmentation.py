...
def no_img_conditioning_augmentation(
    images: torch.Tensor, prob: float = 0.0
) -> torch.Tensor:
    """
    ...
    """
    turn_off_conditioning_prob = torch.rand(
    ...
    images[:, :-1] = torch.where(
        no_img_conditioning_prob, torch.zeros_like(images[:, :-1]), images[:, :-1]
    )
    return images
